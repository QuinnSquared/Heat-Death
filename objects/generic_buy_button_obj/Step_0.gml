...
for (var i = 0; i < instance_number(generic_buy_button_obj); ++i;){
	...
	if(button_in_charge){
		y_offset = instance_find(generic_buy_button_obj,i).y_offset
	}
...
var correct_y_scale = (clicker_store_obj.sprite_height)/(sprite_get_height(sprite_index)*1.5);

image_xscale = correct_x_scale;
image_yscale = correct_y_scale;
x = clicker_store_obj.x + boarder_margin

y = clicker_store_obj.y + boarder_margin + y_offset + other_elements_height
